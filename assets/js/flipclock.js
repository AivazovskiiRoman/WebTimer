...
(function($) {
  ...
  FlipClock.Time = FlipClock.Base.extend({
    ...
    getDateObject: function() {
      ...
      return new Date(new Date().getTime() + this.getTimeSeconds() * 1e3);
    },
    ...
    getTimeSeconds: function(date) {
      ...
      if (this.time instanceof Date) {
        if (this.factory.countdown) {
          return Math.max(this.time.getTime() / 1e3 - date.getTime() / 1e3, 0);
        } else {
          return date.getTime() / 1e3 - this.time.getTime() / 1e3;
        }
      } else {
        return this.time;
      }
